...
// tied to mapoi_server/maps/tag_definitions.yaml.
...
const tagDefinitionsPath = path.join(repoRoot, 'mapoi_server', 'maps', 'tag_definitions.yaml');

function loadServerSystemTagNames() {
  const doc = parse(readFileSync(tagDefinitionsPath, 'utf8'));
  return doc.tags.map((tag) => tag.name);
}

describe('system tag contract', () => {
  it('keeps WebUI pure helper constants aligned with mapoi_server tag_definitions.yaml', () => {
