...
describe('updateDraggedPosition — ドラッグ移動の working copy 反映 (#239)', () => {
  ...
  it('別 POI 編集中 / フォーム閉なら入力欄を同期しない (pose と dirty は更新)', () => {
  ...
  it('renderList が走り list が再描画される', () => {
    editor.updateDraggedPosition(0, 3, 4);
    expect(editor.listEl.querySelectorAll('.poi-card').length).toBe(editor.pois.length);
  });
});
...
describe('showForm / hideForm / exitEditMode — 編集フォーム開閉と可視性通知 (#240)', () => {
  ...
  it('exitEditMode は編集中なら editingIndex を閉じ hideForm で false 通知する', () => {
