...
- **POI 編集**: POI の追加・編集・削除・保存（mapoi_server に自動リロード通知）。地図上の POI はシングルクリックで選択、ダブルクリックで編集パネルを開く、選択中の POI はドラッグで位置を移動、扇形（yaw 制約）の先端ハンドルをドラッグで向き（yaw）を回転（いずれも Save で確定）
