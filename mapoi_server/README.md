...
| `mapoi/nav/backend_status` | `mapoi_interfaces/NavigationBackendStatus` | Nav2 action / service の readiness summary を 1Hz で配信（#198）。`transient_local` QoS。`backend_ready = goal_ready && route_ready` を真値とし、WebUI / panel はこの値で navigation 操作 UI を gate する |
