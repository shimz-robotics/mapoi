...
- **per-POI `tolerance.yaw` が「yaw を見るか」のつまみ**: ただの通過点は `tolerance.yaw` を大きく (例 `3.14` ≒ 360° で yaw 不問) して fly-through、向きが要る点 (撮影・最終 goal 等) は小さくして姿勢を効かせる。`tolerance.yaw` を実効的に効かせるには (xy と同様) Nav2 の `yaw_goal_tolerance` を `tolerance.yaw` 以下まで下げる (OR の実効許容は緩い方になるため)。
