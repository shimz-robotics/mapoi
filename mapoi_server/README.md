...
| `request_initial_pose` | `RequestInitialPose` | `{map_name, poi_name}` を受け取り `mapoi/initialpose_poi` を publish する唯一の経路 (#211)。空 `poi_name` は clear (subscriber は無視)。非空 `map_name` が現在 map と不一致の場合は publish せず `success=false` (#299) |
