...
4 つの core node (`mapoi_server` / `mapoi_nav_server` / `mapoi_amcl_localization_bridge` / `mapoi_rviz2_publisher`) と、必要に応じてシミュレータ連動 bridge をまとめて起動する bringup launch を提供しています。自前の launch から以下のように include してください:
...
AMCL 互換 localization (`/initialpose` を `geometry_msgs/PoseWithCovarianceStamped` で受ける構成) 向けの localization bridge ノードです (#209)。`mapoi_nav_server` から AMCL adapter を分離して、Navigation backend と Localization backend を独立した仕様で扱えるようにしています。slam_toolbox / NDT / 自前 localization に切替える場合は、本 bridge の代替として同じ topic 仕様を満たす自作 bridge を用意してください（ルート README の「Localization backend 仕様」節参照）。
