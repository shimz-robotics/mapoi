...
- **単発 Go (GOAL モード) も POI 個別 tolerance を尊重 (#261)**: `mapoi` モードでは `mapoi/nav/goal_pose_poi` による単発 Go も **到達 = OR((`tolerance.xy` 進入 ∧ `tolerance.yaw` 一致) ∨ Nav2 SUCCEEDED)** で完了する。robot が半径内かつ姿勢が `tolerance.yaw` 以内に自然に収まっていれば Nav2 の最終姿勢合わせを待たず即完了し (snappy)、姿勢がずれていれば radius では完了せず Nav2 SUCCEEDED (= goal_checker の yaw 判定込み) を待つ。`tolerance.yaw` を実効的に効かせるには (xy と同様) Nav2 の `yaw_goal_tolerance` を `tolerance.yaw` 以下まで下げる必要がある (OR の実効許容は緩い方になるため)。`nav2` モードの Go は従来通り Nav2 `goal_checker` 任せ。
