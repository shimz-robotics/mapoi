...
「存在しない route 名」→ (c) の 1 経路しかカバーできない ((b) の任意異常応答は
実物の mapoi_server では作れない)。(b) と (c) はどちらも mock backend でしか
決定論的に再現できないため、``mapoi_server`` を起動せず ``FakeRoutePoisServer``
(nav2_mocks.py) で ``mapoi/get_route_pois`` を差し替える新規 fixture が必要
(docs/testing-policy.md 3節: 既存ファイルと mock/fixture 構成が異なるため分離)。
