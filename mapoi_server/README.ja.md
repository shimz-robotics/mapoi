...
`MapoiNav2Bridge` クラスの実装は、保守性のため 3 つの translation unit に分割されています
(#345、段階的分割の第1弾): `src/mapoi_nav2_bridge.cpp` (ノード初期化、map switch、
pause/resume、backend status、POI 半径/pause 判定エンジンなど共有の核)、
`src/mapoi_nav2_bridge_route.cpp` (route 走行: `FollowWaypoints` と mapoi 主導 waypoint
到達モード)、`src/mapoi_nav2_bridge_goal.cpp` (単発 goal 走行: `NavigateToPose`) です。
3 ファイルとも単一の `mapoi_nav2_bridge` 実行ファイルにコンパイルされ、この分割は内部
