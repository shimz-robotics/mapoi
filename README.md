...
> **DDS 環境の isolation**: container 内では `RMW_IMPLEMENTATION=rmw_fastrtps_cpp` / `ROS_DOMAIN_ID=0` / `CYCLONEDDS_URI=` を **固定** しています。
> - `network_mode: host` + `ipc: host` で host とリソース共有するため、host shell や別 workspace の DDS 設定 (例: cyclonedds + custom URI) が leak すると container 内 discovery が壊れる事例がありました
> - 固定値で動作が壊れる場合は `docker-compose.yml` を編集するか、`docker compose run` の `--env` で個別 override:
>   - 別 ROS_DOMAIN_ID で multi-robot / parallel simulation したい:
>     `ROS_DISTRO=humble docker compose run --rm --env ROS_DOMAIN_ID=42 dev`
>   - CycloneDDS 構成に切替えて custom config も使う:
>     `ROS_DISTRO=humble docker compose run --rm --env RMW_IMPLEMENTATION=rmw_cyclonedds_cpp --env CYCLONEDDS_URI=file:///path/to/cyclonedds.xml dev`
