...
> - host shell に \`ROS_DISTRO\` が設定されていれば **その値を継承** (例: ROS 2 Humble を source 済みなら humble、Jazzy を source 済みなら jazzy)
> - 未設定なら fallback default は \`humble\`
> - **別 distro を一時的に試す**には \`ROS_DISTRO=jazzy docker compose ...\` のように前置で上書き
> - local image は \`mapoi:dev-${ROS_DISTRO}\` の形 (例: \`mapoi:dev-jazzy\`) で distro 別保存、並行運用可
