...
```sh
# 推奨: .env を生成して docker compose に自動 inject (.gitignore 済)
cp .env.example .env
...
`.env` 作成後は build / run / up すべての compose コマンドで自動的に host UID が反映されます。distro 別 image (mapoi:dev-${ROS_DISTRO}) ごとに UID は build 時に焼き込まれるため、UID 変更時は対象 distro の image を rebuild してください。
