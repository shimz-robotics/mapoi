...
`mapoi_nav_server` は実質的に Nav2 用の bridge ノードです。独自 navigation スタックを mapoi の UI（WebUI / RViz panel）から扱いたい場合は、bridge ノードを自作して以下の topic 仕様を満たしてください。
...
> 現状の `mapoi_nav_server` は便宜上 `mapoi/initialpose_poi`（`mapoi_interfaces/InitialPoseRequest`）も subscribe して `/initialpose` に配信する AMCL adapter を内包していますが、これは Navigation contract の必須項目ではありません。将来 `mapoi_<localization>_bridge` 系のノードに分離する方針で、別契約 `LocalizationBackendStatus` を定義予定です（#209）。
...
**`NavigationBackendStatus` 各フィールドの埋め方**（minimal contract）:
...
**Localization は別契約**: 自己位置推定（AMCL / SLAM toolbox / 自前 localization など）の readiness は本 contract では扱いません。`mapoi/initialpose_poi` の subscribe や `/initialpose` への配信は将来 `mapoi_<localization>_bridge` 系のノードに分離する方針で、契約は別途 `LocalizationBackendStatus`（仮）として定義予定です（issue #209）。現状の `mapoi_nav_server` は便宜上 AMCL adapter も内包していますが、独自 localization に切り替える場合は `mapoi_nav_server` の `initial_pose_topic` parameter で配信先 topic を変更できます。
