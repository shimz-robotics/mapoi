...
```sh
# 初回ビルド
ROS_DISTRO=humble docker compose build dev

# シェルに入る（ホストの ./ が /ros2_ws/src/mapoi にマウント済み）
ROS_DISTRO=humble docker compose run --rm dev
...
```sh
USER_ID=$(id -u) GROUP_ID=$(id -g) ROS_DISTRO=humble docker compose build dev
```
...
```sh
ROS_DISTRO=humble docker compose -f docker-compose.yml -f docker-compose.gpu.yml up demo
```
