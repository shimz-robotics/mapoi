...
| `mapoi/initialpose_poi` | `mapoi_interfaces/InitialPoseRequest`（map switch 後の初期位置 POI） |
...
| `mapoi/nav/backend_status` | `mapoi_interfaces/NavigationBackendStatus`（readiness summary、`transient_local`） |
...
**`NavigationBackendStatus` 各フィールドの埋め方**:
...
- `backend_present`: bridge ノードが起動していれば常に `true`
- `backend_ready`: 主要操作（goal / route）を **今この瞬間に** 受け付けられるかの真偽値。実装ごとに必要 capability が異なる場合は `goal_ready && route_ready` のように AND を取って算出する
- `goal_ready` / `route_ready` / `switch_map_ready` / `initialpose_ready`: 個別 capability の readiness。WebUI tooltip でユーザーに per-capability 状態を見せるために使う
- `reason`: `backend_ready=false` の時の人間可読な理由文字列（例: `"navigate_to_pose action not available"`）。空文字も許容するが、トラブルシュート支援のために理由を入れることを推奨
