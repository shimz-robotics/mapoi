...
## v0.4.0 migration (mapoi_nav_server → mapoi_nav2_bridge)
...
ROS 2 launch は **未知 arg を fatal にせず default を黙って効かせる**ため、下流 launch から `with_nav_server:=false` を渡している場合は意図せず `mapoi_nav2_bridge` が起動する silent regression になりえます。`grep -rn 'with_nav_server:=\|mapoi_nav_server' --include='*.launch.*'` で旧名参照を一括 audit してから更新してください。

ROS 2 topic / service / 明示宣言 parameter (`maps_path` / `map_name` / `config_file` 等) / Nav2 native action は不変です。

その他の v0.4.0 破壊的変更 (AMCL adapter 分離、system tag hardcode 等) は [`CHANGELOG.rst`](./CHANGELOG.rst) を参照してください。
...
- `PoiEvent` の event 種別簡素化 (`ENTER` / `EXIT` / `STOPPED` / `RESUMED` の 4 種から `VISITED` / `PAUSED_AT` / `EXIT` の 3 種へ、route 走行 + pause タグ連携にスコープを絞る) — [#220](https://github.com/shimz-robotics/mapoi/issues/220) (v0.4.0 リリース直後着手予定)
...
`mapoi_nav2_bridge` は実質的に Nav2 用の bridge ノードです。独自 navigation スタックを mapoi の UI（WebUI / RViz panel）から扱いたい場合は、bridge ノードを自作して以下の topic 仕様を満たしてください。
