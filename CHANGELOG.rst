...
* The Nav2 bridge node has been renamed from ``mapoi_nav_server`` to
  ...
  compatibility alias is provided. Migration:
  ...
  - Note: ROS 2 launch silently ignores unknown args and applies defaults,
    so passing the old ``with_nav_server:=false`` to an updated
...
* AMCL adapter has been split out of the Nav2 bridge node into a new
  ...
  indicator. See the new "Localization backend 仕様" section in
  ``README.md``.
