...
`rejected` と `backend_unavailable` の使い分け: `backend_unavailable` は **Nav2 側**（action server / `/goal_pose` fallback subscriber）が不在で navigation そのものを実行できない場合に限定する。`get_pois_info` service 未 ready・POI 名 typo・landmark POI 指定・空 route など、**mapoi_server 側** の内部 service 未 ready・入力検証で Nav2 の readiness とは無関係にコマンドを reject する経路は `rejected` を使う（`mapoi_nav2_bridge` は現状 `get_route_pois` 呼び出し前に `wait_for_service` を挟んでいないため、route コマンドの service 未 ready は明示的な reject 経路になっていない。goal 側 (`get_pois_info`) との非対称は既知のギャップ）。運用上どちらも「今回のコマンドは実行されなかった」点は同じだが、`backend_unavailable` を Nav2 readiness 専用に保つことで、WebUI の `Navigation connected` バッジ (`mapoi/nav/backend_status`) との対応関係を崩さない。
