...
- **(a) 致命核**: 誤動作が走行安全やデータ破壊に直結する判定ロジック（例: 初期姿勢決定、地図切替時の初期化タイミング、システムタグ整合性）。unit test で厚く gate し、実装変更のたびに regression を確実に検知できることを優先する
...
gtest / pytest の target 数は `ros-test.yml` の `EXPECTED_TARGETS` が CMakeLists.txt 上の登録数（`ament_(auto_)add_gtest` / `ament_add_pytest_test`）から自動算出する（#351）。target 数の管理は自動化済みのため、この表の数字が古くなっても CI 側の検知は drift しない。表の数字はあくまで現状把握用の目安。
