...
| `camera_node` | `mapoi/events` の `EVENT_PAUSED` を受けて `capture_trigger` POI で撮影 (mock) (#88) |
...
`mapoi/events` の PoiEvent (`EVENT_ENTER` / `EVENT_PAUSED` / `EVENT_EXIT`) を受けて custom tag に応じたアクションを実行する参照実装です。route 走行中の特定 POI で「音声ガイド再生」「撮影」等のシナリオを組む際の雛形になります。
...
`turtlebot3_world` map の `tour_full` route を走行 (`audio_info` 付き `model_exhibit` / `capture_trigger` + `pause` 付き `conference_room` を含む):
...
期待 log:
- `model_exhibit` 通過時 → `[AUDIO_GUIDE] play: poi='model_exhibit' ...` (audio_guide_node)
- `conference_room` 到達時 → 自動 pause → cmd_vel dwell → `[CAMERA] capture: trigger_poi='conference_room' ...` (camera_node)

### custom tag 命名 guideline

mapoi の **custom tag (system tag 以外のユーザー定義タグ)** は以下の命名規約を推奨します。本 sample もこれに従っています:

| 命名パターン | 意味 | 例 |
|---|---|---|
| `<action>_trigger` | この POI で起動するアクション | `capture_trigger` (撮影)、`audio_info` (音声ガイド再生、本 sample は既存 yaml 命名を尊重して `audio_info` のまま — 規約に厳密に従うなら `audio_trigger` 相当の semantic) |
| `<action>_target` | アクション対象として参照される POI | `capture_target` (撮影対象、landmark で参照) |
| `<state>` | POI が表す状態カテゴリ | `hazard` / `atrium` / `inspection` |

### 実装ポイント (subscriber 側)

- `EVENT_ENTER` は **route 走行中 + route 登録 POI** のみ発火する (#220 仕様)。free-roam (IDLE / GOAL mode) では発火しないため、route ベースの動作シナリオ前提
- `EVENT_PAUSED` は **`pause` タグ付き POI 内で navigation が停止 (cmd_vel dwell)** した瞬間に発火、1 visit につき 1 回のみ。`capture_trigger` で撮影したい POI には `pause` タグも併せて付与する
- `EVENT_EXIT` で各 POI の per-visit 状態が reset される (再 visit で再発火)
- 撮影対象 POI (`capture_target` 等) を取りに行く実装例は scope 外 (本 sample は mock log のみ)。実用では `get_pois_info` service で POI list を fetch して tag フィルタする

詳細は [mapoi_interfaces の README](../mapoi_interfaces/README.md#poieventmsg) と [PoiEvent.msg](../mapoi_interfaces/msg/PoiEvent.msg) のヘッダコメントを参照。
