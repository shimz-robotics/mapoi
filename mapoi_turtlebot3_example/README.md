...
### 撮影シナリオの全体像 (camera_node 本番化 path)

`pause` + `capture_trigger` + `capture_target` の組合せで「停止して landmark を向いて撮影」シナリオを構築できます。本 sample (`camera_node`) は mock 実装ですが、実機運用では以下のフローを想定:
