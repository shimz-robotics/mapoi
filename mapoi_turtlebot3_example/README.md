...
4. **`tutorial_03_pause`** — `pause` タグの動作 (default は **手動 resume**、`mapoi_nav2_bridge` の `auto_resume_timeout_sec` を正値で起動時のみ auto-resume が opt-in で有効)
...
# pause + 手動 resume (default): 停止後に別 shell から
...
| `mid_basic` | `waypoint` | 0.35 / 0.785 | `tutorial_01` / `tour_full` の南側中継点 (pure waypoint) |
| `mid_pause` | `waypoint`, `pause` | 0.30 / 3.14 | `tutorial_03` / `tour_full` の `pause` POI (両 landmark の半径外) |
| `mid_audio` | `waypoint`, `audio_info` | 0.40 / 0.785 | `tutorial_04` / `tour_full` の `audio_info` POI (両 landmark の半径外) |
| `goal` | `waypoint` | 0.40 / **0.10** | `tutorial_01` / `02` / `tour_full` の終点 (両 landmark の北側、**yaw 厳密**) |
| `landmark_listed` | `landmark` | **0.35** / 0.785 | `tutorial_02` / `tour_full` の `route.landmarks` 列挙対象 |
| `landmark_unlisted` | `landmark` | 0.30 / 0.785 | `tutorial_02` の列挙外コントラスト (同 map 上に居ても pub されない) |

> **NOTE**: `tutorial_01_basic` / `tutorial_03_pause` / `tutorial_04_event_tag` は `route.landmarks` を省略しているため、yaml schema が `landmarks` 省略時にも正常動作する暗黙 demo を兼ねる (旧 `tour_short` の landmarks 省略動作 demo はこれらに引き継がれる)。
