...
本 PR では `EVENT_ENTER` を扱う `audio_guide_node` のみ同梱しています。`EVENT_PAUSED` / `EVENT_EXIT` 用 subscriber も同型 (`event_type` の比較値を `EVENT_PAUSED` / `EVENT_EXIT` に差し替えるだけ) で書けるため、別 sample は同梱せず必要になれば follow-up で再導入する方針です。pause / resume のしくみ自体の動作確認は `corridor_a` / `corridor_b` の連続 `pause` で別途行えます。
...
`turtlebot3_world` map の `tour_full` route を走行 (`audio_info` 付き `model_exhibit` を含む):
...
- `corridor_a` / `corridor_b` 到達時 → 自動 pause → cmd_vel dwell → 一定時間後 resume (sample subscriber 無しでも mapoi_nav2_bridge が pause/resume 処理を行う)
...
| 地図名 | シナリオ | 検証できる主な機能 |
| --- | --- | --- |
| `turtlebot3_world` | オフィス見学ツアー (見学客を案内、要所で音声ガイド / 自動停止) | system tag 単独 / 複合、`tolerance.yaw` 厳密 ↔ 緩い、`route.waypoints + landmarks` 両方持ち、連続 `pause`、音声 / 観察対象の custom tag |
...
| `conference_room` | `waypoint` | yaw 厳密な経由点 (`tolerance.yaw=0.10`) |
