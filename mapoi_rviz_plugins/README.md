...
| トピック名 | 型 | 説明 |
| --- | --- | --- |
| `mapoi/initialpose_poi` | `mapoi_interfaces/InitialPoseRequest` | 初期位置に採用する POI 名 (`{map_name, poi_name}`、`transient_local`)。実際の `/initialpose` 配信は `mapoi_amcl_localization_bridge` (#209) が担当 |
| `mapoi/nav/goal_pose_poi` | `std_msgs/String` | ゴール POI 名の配信。実際の Nav2 action 起動は navigation bridge (`mapoi_nav_server` ほか) が担当 |
